...
A hybrid LuaJIT 2.x / Lua 5.1 AOT compiler written in Zig 0.16. It conservatively lowers eligible scalar-numeric chunks to native machine code and automatically routes all dynamic Lua programs through the complete LuaJIT compatibility backend.
...
The selected backend is reported during compilation as `native-numeric` or `luajit-fallback`. Native eligibility deliberately fails closed whenever the compiler cannot prove that direct C `double` operations preserve the supported Lua semantics.
...
- Native lowering currently operates at whole-chunk granularity and supports only conservative scalar arithmetic.
